import { createWord, fetchCategories, fetchRandomWord, fetchWords, type AnswerStyle, type Mode, type WordPayload } from './api.js';
...
import { isFavorite, normalizeAnswer, sampleOptions, saveFavoriteIds, shuffle, state, wordGerman, wordKey, type DeckSession } from './state.js';

const AUTO_ADVANCE_MS = 2000;
const ARTICLE_MODE_STYLE: AnswerStyle = 'multi';
...
function createDeckSession(): DeckSession {
...
function sessionForCurrentDeck(): DeckSession {
  const key = deckKey();
...
function showDeckComplete(): void {
  clearAdvanceTimer();
  state.current = null;
  state.options = [];
  state.selected = null;
...
function renderCurrentAnswerStyle(): void {
...
function acceptedAnswers(): string[] {
  if (!state.current) return [];
  if (state.mode === 'articles') return [normalizeAnswer(state.current.article)];
  if (state.mode === 'english') return [normalizeAnswer(state.current.english)];
  return [normalizeAnswer(wordGerman(state.current)), normalizeAnswer(state.current.german)];
}

function disableCurrentAnswerControls(): void {
  elements.nextButton.disabled = false;
  elements.showAnswerButton.disabled = true;
...
function finishAnswer(correct: boolean, message: string): void {
  state.answered += 1;
...
function chooseAnswer(option: string, button: HTMLButtonElement): void {
  ...
  if (!correct) {
    [...elements.answers.children].forEach((candidate) => {
      if (candidate.textContent === expectedAnswer()) candidate.classList.add('correct');
    });
  }
...
function checkCardAnswer(event: SubmitEvent): void {
...
function formPayload(): WordPayload {
...
async function addCustomWord(event: SubmitEvent): Promise<void> {
  event.preventDefault();
  const submitButton = elements.wordForm.querySelector<HTMLButtonElement>('button[type="submit"]');
